    ...
    <div className={catalogPageShell}>
      ...
      {filtered.length === 0 ? (
        ...
        <ul className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3 lg:gap-10">
          {filtered.map((item) => (
            <li key={item.id}>
              <div className={`${catalogCardLinkClass} flex flex-col`}>
                <Link
                  href={`/item/${item.id}`}
                  className="relative block aspect-square w-full overflow-hidden bg-gradient-to-br from-stone-100 to-smiski-light/35"
                >
                  <Image
                    src={item.image}
                    alt={item.name}
                  ...
                  />
                  {item.isSecret && (
                    <span className={`absolute right-3 top-3 ${secretBadgeClass}`}>
                      Secret
                    </span>
                  )}
                  <span className="absolute bottom-3 left-3 rounded-full border border-white/80 bg-white/95 px-2.5 py-1 text-[11px] font-medium text-stone-600 opacity-0 shadow-sm transition duration-300 group-hover:translate-y-0 group-hover:opacity-100 sm:translate-y-1">
                    View detail →
                  </span>
                </Link>
                <div className="flex flex-1 flex-col gap-3 p-5 sm:p-6">
                  <div className="flex items-start justify-between gap-3">
                    <Link
                      href={`/item/${item.id}`}
                      className="min-w-0 flex-1 focus:outline-none focus-visible:ring-2 focus-visible:ring-smiski-primary/35 focus-visible:ring-offset-2"
                    >
                      <h2 className="text-lg font-semibold leading-snug tracking-tight text-stone-800 transition hover:text-smiski-dark">
                        {item.name}
                      </h2>
                    </Link>
                    <span className="shrink-0 rounded-full border border-smiski-primary/20 bg-smiski-light/80 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-smiski-dark">
                  ...
                  <div className="flex flex-wrap gap-2 border-t border-stone-100 pt-4">
                    {item.tags.slice(0, 4).map((tag) => (
                      <span
                        key={tag}
                        className="rounded-full border border-smiski-primary/20 bg-smiski-light/90 px-2.5 py-0.5 text-xs font-medium text-smiski-dark"
                      >
                        {tag}
                      </span>
                    ))}
